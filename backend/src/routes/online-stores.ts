...
import { onlineProductRepository } from '../repositories/online-product-repository';
...
router.post('/', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existingSlug = await queryOne(
      'SELECT id FROM OnlineStores WHERE slug = @slug',
      { slug: finalSlug }
    );
    ...
    const parentStore = await queryOne<{ name: string; phone: string; address: string }>(
      'SELECT name, phone, address FROM Stores WHERE id = @storeId',
      { storeId }
    );
...
router.delete('/:id', async (req: AuthRequest, res: Response) => {
  try {
    ...
    await query('DELETE FROM OnlineProducts WHERE online_store_id = @id', { id });
    await query('DELETE FROM OnlineOrders WHERE online_store_id = @id', { id });
    await query('DELETE FROM ShoppingCarts WHERE online_store_id = @id', { id });

    // Delete the online store
    await query('DELETE FROM OnlineStores WHERE id = @id AND store_id = @storeId', { id, storeId });
...
router.put('/:id', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const storeId = req.storeId!;
    const {
      slug, customDomain, isActive, storeName, logo, favicon, description,
      themeId, primaryColor, secondaryColor, fontFamily, contactEmail,
      contactPhone, address, facebookUrl, instagramUrl, currency, timezone
    } = req.body;
...
router.get('/:id/products', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const products = await onlineProductRepository.findAllWithDetails(id);
    res.json(products);
  } catch (error) {
...
router.post('/:id/sync', async (req: AuthRequest, res: Response) => {
  try {
    ...
    for (const product of products) {
      ...
      const existing = await onlineProductRepository.findByProductId(product.id, id);
      ...
      while (!(await onlineProductRepository.isSlugAvailable(slug, id))) {
    ...
    const categoryName = categoryId 
      ? (await queryOne<{name: string}>('SELECT name FROM Categories WHERE id = @categoryId', { categoryId }))?.name 
      : 'tất cả';
...
router.post('/:id/products', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const product = await queryOne<{ id: string; name: string; price: number }>(
      'SELECT id, name, price FROM Products WHERE id = @productId AND store_id = @storeId',
      { productId, storeId }
    );
    ...
    const existing = await onlineProductRepository.findByProductId(productId, id);
    if (existing) {
...
router.delete('/:id/products/:productId', async (req: AuthRequest, res: Response) => {
  try {
    ...
    await onlineProductRepository.delete(productId, id);
    res.json({ success: true });
  } catch (error) {
...
router.put('/:id/products/:productId', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const updated = await onlineProductRepository.update(productId, {
      isPublished,
      onlinePrice,
      onlineDescription,
      seoSlug,
