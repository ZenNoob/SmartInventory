...
const DEFAULT_CONFIG: TenantRouterConfig = {
  maxPoolSize: 10,
  minPoolSize: 0,
  idleTimeoutMs: 30000,
...
export class TenantRouter {
  ...
  private masterPool: sql.ConnectionPool | null = null;
  private config: TenantRouterConfig;
  private cleanupInterval: NodeJS.Timeout | null = null;
  private tenantInfoCache: Map<string, { info: TenantInfo; cachedAt: Date }> = new Map();
  ...
  async initialize(): Promise<void> {
    ...
    this.masterPool = await sql.connect({
   ...
   */
  private async createTenantPool(tenantInfo: TenantInfo): Promise<sql.ConnectionPool> {
   ...
   */
  getActiveConnections(): { tenantId: string; databaseName: string; lastAccessed: Date }[] {
